...
class TemporalEncoder(nn.Module):                 
    ...
    def forward(self, x, is_train=False): 
        ...
        xtemp = self.linear_in(x.permute(0,2,1) )
        #16*32*2048
        mul_theta_phi = self.vit(xtemp)
        #32*16*256
        mul_theta_phi = torch.matmul(mul_theta_phi, mul_theta_phi.permute(0, 2, 1))
        #mul_theta_phi = self.softmax(mul_theta_phi)                                                   	# N x 16 x 16
        Y = self.softmax(mul_theta_phi)  
...
def test_net():
    ...
    print('transformer:',flops, params)
    model.eval()
