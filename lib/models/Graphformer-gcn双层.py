...
class TemporalEncoder(nn.Module):
    ...
    def forward(self, x, is_train=False):
        ...
        y_bef = x[:, :self.mid_frame]
        y_aft = x[:, self.mid_frame+1:]

        for i in range(2):
            res_cur = y_cur
            y_cur = self.gcn16(y_cur)
            y_cur = self.layer_norm(y_cur + res_cur)
            y_cur = self.ffn16(y_cur.permute(0,2,1)).permute(0,2,1)
            
            res_bef = y_bef
            y_bef = self.gcn8(y_bef)
            y_bef = self.layer_norm(y_bef + res_bef)
            y_bef = self.ffn8(y_bef.permute(0,2,1)).permute(0,2,1)
            
            res_aft = y_aft
            y_aft = self.gcn7(y_aft)
            y_aft = self.layer_norm(y_aft+res_aft)
...
# 'mpjpe': 87.9345, 
# 'mpjpe_pa': 52.605724*,
# 'accel_err': 8.56295460941834, 
# 'mpvpe': 104.81837}
#mpii3d {
# 'mpjpe': 98.692635, 
# 'mpjpe_pa': 62.692184*, 
# 'accel_err': 9.965223115008227}
#h36m{
# 'mpjpe': 72.11291*, 
# 'mpjpe_pa': 50.414062*, 
# 'accel_err': 4.730827929049915}
