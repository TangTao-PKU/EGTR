...
from lib.models.mutiheadattention import MultiHeadAttention
import math
...
class TemporalEncoder(nn.Module):
    def __init__(
        ...
        self.linear_in  = nn.Linear(2048,512)
        self.linear_out  = nn.Linear(512,2048)
        self.attn = nn.MultiheadAttention(512, 8, dropout=0.1)


    def forward(self, x, is_train=False):
        att = self.linear_in(x)
        #print(att.shape)
        att, _ = self.attn(att, att, att)
        ...
        #self.attention
        y_cur = self.gcn16(y_cur) + x_cur + att
...
def test_net():
    ...
    # input_2d = torch.randn(batch_size, 16, 2048).cuda()
...
# python evaluate.py --cfg ./configs/repr_table4_3dpw_model.yaml --gpu 0 --dataset 3dpw
...
#gcn+attn
#3DPW {'mpjpe': 88.86922, 'mpjpe_pa': 53.16309, 'accel_err': 7.378822312572603, 'mpvpe': 104.55908}
#mpii3d {'mpjpe': 102.98308, 'mpjpe_pa': 65.03666, 'accel_err': 8.94913952938495}
#h36m {'mpjpe': 72.69714, 'mpjpe_pa': 51.289722, 'accel_err': 3.887766428665812}

#gcn+attn
# {'mpjpe': 89.7187, 'mpjpe_pa': 53.47313, 'accel_err': 7.28983409856296, 'mpvpe': 105.40135}
# {'mpjpe': 103.739235, 'mpjpe_pa': 65.252975, 'accel_err': 8.877582234715963}
# {'mpjpe': 73.12834, 'mpjpe_pa': 51.471973, 'accel_err': 3.8114759417244954}
