...
# ------------------------------#
# Positional Encoding�Ĵ���ʵ��
# ------------------------------#
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=16):
        super(PositionalEncoding, self).__init__()

        # λ�ñ����ʵ����ʵ�ܼ򵥣�ֱ�Ӷ����Ź�ʽȥ�ô���Ϳ��ԣ�����Ĵ���ֻ�����е�һ��ʵ�ַ�ʽ��
        # ��������������Ҫע��ľ���ż���������ڹ�ʽ����һ����ͬ���֣�����ʹ��log�����Ѵη���������������㣻
        # pos�������ǵ����ھ����е������������Ҫע�⣻����max_len��128������ô�������Ǵ�0��1��2��...,127
        # �����ҵ�d_model��512��2i�Բ���2��0ȡ����512����ôi��Ӧȡֵ����0,1,2...255
        self.dropout = nn.Dropout(p=dropout)

        pe = torch.zeros(max_len, d_model)
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        pe[:, 0::2] = torch.sin(position * div_term)  # ������Ҫע�����pe[:, 0::2]����÷������Ǵ�0��ʼ������棬����Ϊ2����ʵ�����ľ���ż��λ��
        pe[:, 1::2] = torch.cos(position * div_term)  # ������Ҫע�����pe[:, 1::2]����÷������Ǵ�1��ʼ������棬����Ϊ2����ʵ�����ľ�������λ��

        # �������д���֮�����ǵõ���pe��״�ǣ�[max_len * 1 * d_model]
        pe = pe.unsqueeze(0).transpose(0, 1)

        self.register_buffer('pe', pe)  # ��һ������������ʵ������Ϊ������������¾Ϳ���
...
def test_net():
    batch_size = 1
    model = TCMR(seqlen=16, batch_size=batch_size, n_layers=1, hidden_size=1024) # ģ�ͳ�ʼ��
    model = model.cuda()
    ...
    input = torch.randn(batch_size, 16, 2048).cuda() # ����GPU
    # print("input:",input)
