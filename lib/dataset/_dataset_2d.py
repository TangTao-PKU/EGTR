...
class Dataset2D(Dataset):
    ...
    def load_db(self):
        ...
        db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{set}_db.pt')
        if self.set == 'train':
            ...
            #     db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{set}_scale12_db.pt')

        if osp.isfile(db_file):
            db = joblib.load(db_file)
        else:
            raise ValueError(f'{db_file} 不存在')
