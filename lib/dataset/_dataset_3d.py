...
from lib.dataset._img_utils import normalize_2d_kp, transfrom_keypoints, split_into_chunks, get_single_image_crop
...
class Dataset3D(Dataset):
    ...
    def __getitem__(self, index):
        #### 重写dataset迭代器 每次返回index帧邻近16帧16*2048  ####
        return self.get_single_item(index)

    def load_db(self):

        db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_db.pt')

        if self.set == 'train':
            if self.load_opt == 'Graphformer-config':
                if self.dataset_name == '3dpw':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_occ_db.pt')
                elif self.dataset_name == 'mpii3d':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_scale12_occ_db.pt')
                elif self.dataset_name == 'h36m':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_25fps_occ_db.pt')

            elif self.load_opt == 'repr_table4_h36m_mpii3d_model':
                if self.dataset_name == '3dpw':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_db.pt')
                elif self.dataset_name == 'mpii3d':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_scale12_db.pt')
                elif self.dataset_name == 'h36m':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_25fps_db.pt')

            elif self.load_opt == 'repr_table6_3dpw_model':
                if self.dataset_name == 'mpii3d':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_scale12_new_occ_db.pt')
                elif self.dataset_name == 'h36m':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_25fps_occ_db.pt')

            elif self.load_opt == 'repr_table6_h36m_model':
                if self.dataset_name == 'mpii3d':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_scale1_db.pt')
                elif self.dataset_name == 'h36m':
                    db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_{self.set}_25fps_tight_db.pt')
            ...
            if self.load_opt == 'repr_table6_h36m_model':
                db_file = osp.join(Graphformer_DB_DIR, f'{self.dataset_name}_test_front_25fps_tight_db.pt')
        ...
        print(f'载入 {self.dataset_name} 数据集文件 {db_file}')
        return db
    ...
    def get_single_item(self, index):
        # 重写迭代器函数 每个获取16*2048 daraloader获取bacthsize个
