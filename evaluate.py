...
from lib.dataset._img_utils import split_into_chunks
...
if __name__ == "__main__":
    ...
    if target_dataset == '3dpw':
        data_path = f'./data/preprocessed_data/3dpw_test_db.pt'
    elif target_dataset == 'mpii3d':
        set = 'val'
        data_path = f'./data/preprocessed_data/{target_dataset}_{set}_scale12_db.pt'  #
    elif target_dataset == 'h36m':
        data_path = f'./data/preprocessed_data/h36m_test_front_25fps_tight_db.pt'
    else:
        print("错误数据集...")
        import sys; sys.exit()

    print(f"导入测试数据集{data_path}")
    ...
    with torch.no_grad():
        ...
        for seq_name in pbar:
            ...
            if render:
                ...
                save_seq_name = 'Graphformer_' + save_seq_name + '_' + str(render_frame_start)
            ...
            print(f"在 {num_eval_pose} 个姿态上测试,来自{seq_name}...")
